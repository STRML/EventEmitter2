...
;!function(undefined) {
  ...
  EventEmitter.prototype.off = function(type, listener) {
      ...
      if (Array.isArray(handlers)) {
        ...
        if (handlers.length === 0) {
          if (this.wildcard) {
            ...
            this._reverseGarbageCollect(leaf);
          }
          else {
        ...
        // FIXME remove this branch, should not be possible
        if(this.wildcard) {
    ...
    if (this.wildcard) {
      console.log('removeListener', type);
      this._reverseGarbageCollect(type);
    }
  ...
  EventEmitter.prototype._reverseGarbageCollect = function(type) {
    if (!type) return;

    // Go up one node, check if empty, if so delete
    const ns = typeof type === 'string' ? type.split(this.delimiter) : type.slice();
    const originalNS = ns.slice();

    let leafKey;
    let node = this.listenerTree;
    let nodePath = [this.listenerTree];
    for (let i = 0; i < ns.length - 1; i++) {
      node = node[ns[i]];
      if (!node) return this;
      nodePath.push(node);
    }
    console.log('blowing up', type);
    while ((leafKey = ns.pop())) {
      console.log({leafKey});
      // Get deep path
      const innerNode = nodePath[ns.length];

      // Get all other nodes at this level if we're removing a wildcard.
      // We recursively call this function in case this is something like "foo.*.bar", so we can't
      // get away with just deleting the entire level.
      if (leafKey === '*') {
        const idx = ns.length;
        for (const key in innerNode) {
          if (key !== '*') {
            this._reverseGarbageCollect(originalNS.slice(0, idx).concat([key]).concat(originalNS.slice(idx + 1)));
          }
        }
      } else if (leafKey === '**') {
        // Delete everything at this level. Emptying out the object is easiest so we don't have to play
        // tricks with state.
        for (const key in innerNode) {
          delete innerNode[key];
        }
      }

      if (innerNode[leafKey] && innerNode[leafKey]._listeners.length === 0) {
        delete innerNode[leafKey];
      } else if (leafKey !== '*' && leafKey !== '**') {
        break; // we know we can't GC the rest then
      }
    }
